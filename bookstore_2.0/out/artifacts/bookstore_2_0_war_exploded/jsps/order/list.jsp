...
<html>
  ...
  <body>
  	<div align="center"><h3>我的订单</h3></div>
						...
						<div style="float:right;margin-right:10px;">
							<c:choose>
								<c:when test="${order.state eq 1 }">
									<a href="<c:url value='/OrderServlet?method=load&oid=${order.oid }'/>" style="color:red;">去付款</a>
								</c:when>
								<c:when test="${order.state eq 2 }">
									<span>待发货</span>
								</c:when>
								<c:when test="${order.state eq 3 }">
									<a href="<c:url value='/OrderServlet?method=confirm&oid=${order.oid }'/>" style="color:#7fb80e;">确认收货</a>
								</c:when>
